...
## SSE keepalive: why proxies kill long-running streams

Server-Sent Events (SSE) hold an HTTP connection open indefinitely. But most
reverse proxies (nginx, Cloudflare, HuggingFace's load balancer) have an **idle
timeout** — if no bytes flow for N seconds, the proxy closes the connection.

For the agent this is a real problem: tool calls take 5-15s each, so a 4-iteration
run easily exceeds a 30s idle timeout mid-stream.

The fix: **SSE comment lines**. The SSE spec allows lines starting with `:` as
comments — the browser ignores them but they count as traffic, resetting the
proxy's idle timer:
...
We send one every 15 seconds if no real event has arrived. The browser never
sees it; the proxy thinks the connection is active.

---

## asyncio.wait_for danger with async generators

The naive keepalive implementation looks like this:
...
**This is broken.** `asyncio.wait_for` cancels the coroutine on timeout. When
it cancels `gen.__anext__()`, it injects `CancelledError` into the generator's
`await` point — corrupting its internal state. The next `__anext__()` call hangs
forever because the generator is stuck mid-execution.

The fix: **queue-based producer pattern**. Run the generator in a separate task
that pushes events to a queue. Wait on the queue with a timeout instead:
...
`queue.get()` being cancelled is safe — the item stays in the queue and gets
picked up on the next iteration. The producer task is never interrupted.

Key insight: `asyncio.wait_for` with a timeout is safe on **queue operations**
but dangerous on **generator iteration**. Queue operations are atomic; generator
iteration holds internal state across `await` points.

---

## localStorage vs sessionStorage for caching LLM output

When diagrams and concept tours are generated by an LLM, they're expensive:
a diagram might cost 3-5 seconds and a real API call. Re-generating on every
page refresh wastes quota and slows the UX.

Two options:
- `sessionStorage` — survives page refresh, wiped when the tab closes
- `localStorage` — survives tab close and browser restart, never wiped unless explicitly cleared

For LLM-generated content that doesn't change (a code architecture diagram
doesn't change unless the repo is re-indexed), `localStorage` is better. We
only clear it when the user clicks "Regenerate".

```js
// Write on success
localStorage.setItem(`ghrc_diagram_${repo}`, JSON.stringify(data));

// Read on load — skip the API call if cached
const cached = localStorage.getItem(`ghrc_diagram_${repo}`);
if (cached) return JSON.parse(cached);

// Clear on explicit regenerate
localStorage.removeItem(`ghrc_diagram_${repo}`);
```

---

## Per-request model override with save/restore

The agent service initialises once with a default client/provider/model
from the priority chain. But we want users to be able to select a different
model per-request without affecting the default for the next request.

The pattern: save, override, restore in a `finally` block.

```python
async def stream(self, question, model_id=None):
    # Save current defaults
    _orig = (self._client, self._provider, self._model)

    # Override if user selected a specific model
    ...
    try:
        ...
        # Always restore — even if an exception was raised mid-stream
        self._client, self._provider, self._model = _orig
```

The `finally` block is critical: without it, if an error occurs mid-stream, the
service is left in the overridden state and every subsequent request uses the
wrong model.
...
Rather than hardcoding model names throughout the codebase, we define a catalog:

```python
AGENT_MODELS = [
    {
        "id":          "cerebras/qwen3-235b",   # stable ID for API/localStorage
        "name":        "Qwen3 235B",            # display name
        "provider":    "cerebras",
        "model":       "qwen-3-235b-a22b-instruct-2507",  # actual API name
        "requires":    "cerebras_api_key",      # settings attribute to check
        "speed":       "fast",
        "speed_label": "~40s",
        "note":        "Best balance. Fast inference...",
    },
    ...
]
```

The catalog is the single source of truth. The `/agent/models` endpoint reads
it and adds `available: bool` based on whether the API key is configured.
The frontend uses `id` as the stable key (persisted to localStorage), never
the raw model name which can change between versions.
